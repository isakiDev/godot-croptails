...
#	call the func after de current frame is processing
#	so, call_deferred is called during idle state
...
#	await get_tree().physics_frame
	...
	await navigation_agent_2d.ready
	set_movement_target()
...
#	random speed to each characters
...
func _on_physics_process(_delta: float) -> void:
	...
	#	we not use delta because its used inside the method move_and_slide
